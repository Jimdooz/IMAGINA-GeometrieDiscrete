...
function draw(Time) {
  ...
  for(let i = 0; i < m.hedges.length; i++){
    let e = m.getHEdge(i);
    ctx.fillText(`HEdge ${i} : incident = ${e.vhead} ; suivant = ${e.next} ; précèdent = ${e.prev} ; oppposé = ${e.opposite} ; face = ${e.face} ;`, 20, ctx.canvas.height + i * 16 - 20 - ((m.hedges.length - 1) * 16));
  }
}
